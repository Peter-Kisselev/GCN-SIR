        ...
        for mode in modes:
            ...
            for x_od, x_node, x_SIR, y_true in data_loader[mode]:
                ...
                if mode == "full":
                    beta = beta[0][-1]
                    gamma = gamma[0][-1]
                    print(max(beta[n][0] for n in range(len(gamma))), max(gamma[n][0] for n in range(len(gamma))))

                    K = []
                    print(min(sum(x_SIR[0][-1][n]) for n in range(len(x_SIR[0][-1]))))
                    for n in range(len(beta)):
                        nn = sum(x_SIR[0][-1][n])
                        bn = beta[n][0]
                        gn = gamma[n][0]
                        # cm = sum((commute[n2][m]/sum(x_SIR[0][-1][n2]) + commute[m][n2]/sum(x_SIR[0][-1][m])) for n2 in range(len(beta)))
                        row = []
                        for m in range(len(beta)):
                            amn = (commute[n][m]/sum(x_SIR[0][-1][n]) + commute[m][n]/sum(x_SIR[0][-1][m]))
                            row.append(float(bn*nn*amn/gn))
                        K.append(row)
                        # print(row)
                    ...
                    print(min(abs(eigenvalues)))
                    r0 = max(abs(eigenvalues)/min(abs(eigenvalues)))
                    print(r0)
...
def main():
    ...
    if not bool(params['test_only']):
        trainer.train(data_loader=data_loader, modes=['train', 'validate'], early_stop_patience=50)
