...
class ODDataset(Dataset):
    ...
    def prepare_xy(self, inputs: dict, output: torch.Tensor):
        if self.mode == 'train' or self.mode == 'full':
            start_idx = 0
...
class DataGenerator(object):
    ...
    def get_data_loader(self, data: dict, params: dict):
        ...
        for i in range(x_node.shape[-1]):
                scaler = StandardScaler(mean=x_node[:mode_len['train'],..., i].mean(), std=x_node[:mode_len['train'],..., i].std())
        ...
        for mode in ['train', 'validate', 'test', 'full']:
            dataset = ODDataset(inputs=feat_dict, output=y, mode=mode, mode_len=mode_len)
            print('Data loader', '|', mode, '|', 'input node features:', dataset.inputs['x_node'].shape, '| output:', dataset.output.shape)
            if mode == 'train':
