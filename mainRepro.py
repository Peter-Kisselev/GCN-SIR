...
class ModelTrainer(object):
    def __init__(self, params: dict, data: dict, data_container, data_loader):
        self.params = params
    ...
    def get_model(self):
        return mepoGNN.mepognn(num_nodes = self.params['N'], in_dim=3, blocks=2, layers=3, adpinit=self.commuter, in_len=self.params['obs_len'], out_len=self.params['pred_len'], dropout=0.5)

    def get_loss(self):
        if self.params['loss'] == 'MSE':    criterion = nn.MSELoss()
        elif self.params['loss'] == 'MAE':  criterion = nn.L1Loss()
        return criterion

    def get_optimizer(self):
        return optim.Adam(params=self.model.parameters(), lr=self.params['learn_rate'], weight_decay=self.params['weight_decay'])
            ...
            for mode in modes:
                ...
                for x_od, x_node, x_SIR, y_true in data_loader[mode]:
                    with torch.set_grad_enabled(mode=(mode == 'train')):
                        ...
                        if mode == 'train':
                            ...
                            if epoch % 2 == 0 and self.task_level <= self.params['pred_len']: self.task_level += 1

                            if self.cl:
                                loss = self.criterion(y_pred[:, :self.task_level, :, :], y_true[:, :self.task_level, :, :])
                            else:
                ...
                if mode == 'validate':
                    ...
                    if epoch_val_loss <= val_loss:
                        print(f'Epoch {epoch}, validation loss drops from {val_loss:.5} to {epoch_val_loss:.5}. Update model checkpoint..', f'used {(datetime.now() - starttime).seconds}s')
    ...
    def test(self, data_loader: dict, modes: list):
        ...
        self.model.eval()
        commute = np.array(self.commuter.to("cpu"))

        for mode in modes:
            print('\n', datetime.now().strftime('%Y/%m/%d %H:%M:%S'))
            print(f'     {self.params["model"]}-US model testing on {mode} data begins:')
            ...
            for x_od, x_node, x_SIR, y_true in data_loader[mode]:
                y_pred, beta, gamma = self.model(x_node.transpose(1, 3), x_SIR, None, None, retPars=True)
                print("Beta: " + str(beta.shape) + "Gamma: " + str(gamma.shape))
                ...
                if mode == "full":
                    K = []
                    for n in range(len(beta[0][-1])):
                        row = []
                        for m in range(len(beta[0][-1])):
                            amn = (commute[n][m]/sum(x_SIR[0][-1][n]) + commute[m][n]/sum(x_SIR[0][-1][m]))
                            row.append(float(amn))
                        K.append(row)
                    ...
                    for t in range(len(beta)):
                        ...
                        A = np.diagflat([float(2*betaNew[n][0]/gammaNew[n][0]) for n in range(len(betaNew))])
                        aArr.append(A)
                        ...
                        # print(r0arr)
            np.save("mepoUS/Data/countyDataClean/"+self.params["model"]+"-reproductionNumStates.npy",hArr)
            np.save("mepoUS/Data/countyDataClean/"+self.params["model"]+"-reproductionNumStates2.npy",aArr)
            np.save("mepoUS/Data/countyDataClean/"+self.params["model"]+"-commute.npy",commute)
            np.save("mepoUS/Data/countyDataClean/"+self.params["model"]+"-reproductionNum.npy",r0arr)
            ...
            np.save(self.params['output_dir'] + '/' + self.params['model'] + '_' + mode + '_groundtruth.npy', ground_truth)
...
def main():
    ...
    parser.add_argument('-GPU', '--GPU', type=str, help='Specify GPU usage', default='cuda')
    parser.add_argument('-in', '--input_dir', type=str, default='../data')
    parser.add_argument('-out', '--output_dir', type=str, default='./output')
    parser.add_argument('-model', '--model', type=str, help='Specify model', choices=['MepoGNN'], default='MepoGNNTEST')
    ...
    parser.add_argument('-split', '--split_ratio', type=float, nargs='+', help='Relative data split ratio in train : validate : test, Example: -split 6 1 1', default=[6, 1, 1])
    ...
    parser.add_argument('-rep', '--reproduce', type=bool, default=False)
    parser.add_argument('-rd', '--random_seed', type=int, default=5)
    ...
    # get model
    trainer = ModelTrainer(params=params, data=data, data_container=data_input, data_loader=data_loader)

    print("\n Ready to train \n")

    if not bool(params['test_only']):
        trainer.train(data_loader=data_loader, modes=['train', 'validate'], early_stop_patience=20)
    trainer.test(data_loader=data_loader, modes=['train', 'validate', 'test', 'full'])
